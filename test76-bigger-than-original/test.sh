...
do
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours1/300 -filter Triangle -define filter:support=2.0 -thumbnail 300 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours1/600 -filter Triangle -define filter:support=2.0 -thumbnail 600 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours1/1200 -filter Triangle -define filter:support=2.0 -thumbnail 1200 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours2/300 -filter Triangle -define filter:support=2.0 -thumbnail 300 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours2/600 -filter Triangle -define filter:support=2.0 -thumbnail 600 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours2/1200 -filter Triangle -define filter:support=2.0 -thumbnail 1200 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours3/300 -filter Triangle -define filter:support=2.0 -thumbnail 300 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours3/600 -filter Triangle -define filter:support=2.0 -thumbnail 600 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours3/1200 -filter Triangle -define filter:support=2.0 -thumbnail 1200 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours4/300 -filter Triangle -define filter:support=2.0 -thumbnail 300 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours4/600 -filter Triangle -define filter:support=2.0 -thumbnail 600 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours4/1200 -filter Triangle -define filter:support=2.0 -thumbnail 1200 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours5/300 -filter Triangle -define filter:support=2.0 -thumbnail 300 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours5/600 -filter Triangle -define filter:support=2.0 -thumbnail 600 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours5/1200 -filter Triangle -define filter:support=2.0 -thumbnail 1200 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours6/300 -filter Triangle -define filter:support=2.0 -thumbnail 300 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours6/600 -filter Triangle -define filter:support=2.0 -thumbnail 600 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours6/1200 -filter Triangle -define filter:support=2.0 -thumbnail 1200 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours7/300 -filter Triangle -define filter:support=2.0 -thumbnail 300 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours7/600 -filter Triangle -define filter:support=2.0 -thumbnail 600 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours7/1200 -filter Triangle -define filter:support=2.0 -thumbnail 1200 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours8/300 -filter Triangle -define filter:support=2.0 -thumbnail 300 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours8/600 -filter Triangle -define filter:support=2.0 -thumbnail 600 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours8/1200 -filter Triangle -define filter:support=2.0 -thumbnail 1200 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours9/300 -filter Triangle -define filter:support=2.0 -thumbnail 300 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours9/600 -filter Triangle -define filter:support=2.0 -thumbnail 600 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours9/1200 -filter Triangle -define filter:support=2.0 -thumbnail 1200 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours10/300 -filter Triangle -define filter:support=2.0 -thumbnail 300 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours10/600 -filter Triangle -define filter:support=2.0 -thumbnail 600 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path colours10/1200 -filter Triangle -define filter:support=2.0 -thumbnail 1200 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -colors $NUMBER_OF_COLORS -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path type/300 -filter Triangle -define filter:support=2.0 -thumbnail 300 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -type $TYPE -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path type/600 -filter Triangle -define filter:support=2.0 -thumbnail 600 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -type $TYPE -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path type/1200 -filter Triangle -define filter:support=2.0 -thumbnail 1200 -unsharp 0.25x0.08+8.3+0.045 -dither None -posterize 136 -quality 82 -type $TYPE -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path type-dither/300 -filter Triangle -define filter:support=2.0 -thumbnail 300 -unsharp 0.25x0.08+8.3+0.045 -posterize 136 -quality 82 -type $TYPE -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path type-dither/600 -filter Triangle -define filter:support=2.0 -thumbnail 600 -unsharp 0.25x0.08+8.3+0.045 -posterize 136 -quality 82 -type $TYPE -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path type-dither/1200 -filter Triangle -define filter:support=2.0 -thumbnail 1200 -unsharp 0.25x0.08+8.3+0.045 -posterize 136 -quality 82 -type $TYPE -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path type-noposter-dither/300 -filter Triangle -define filter:support=2.0 -thumbnail 300 -unsharp 0.25x0.08+8.3+0.045 -quality 82 -type $TYPE -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path type-noposter-dither/600 -filter Triangle -define filter:support=2.0 -thumbnail 600 -unsharp 0.25x0.08+8.3+0.045 -quality 82 -type $TYPE -define png:preserve-colormap=true -interlace none $f
	fi
	...
	if [ "$resizeSize" -gt "$controlSize" ]; then
		echo "redoing $resize"
		mogrify -path type-noposter-dither/1200 -filter Triangle -define filter:support=2.0 -thumbnail 1200 -unsharp 0.25x0.08+8.3+0.045 -quality 82 -type $TYPE -define png:preserve-colormap=true -interlace none $f
	fi
done
